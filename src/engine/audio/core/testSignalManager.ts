...
export class TestSignalManager {
    ...
    async start(type: 'tone' | 'noise' | 'impulse', logicInputId: string, options: TestSignalOptions = {}): Promise<void> {
        ...
        try {
            switch (type) {
                case 'tone':
                    this.createToneSignal(spec, options);
                    break;
                case 'noise':
                    this.createNoiseSignal(spec, options);
                    break;
                case 'impulse':
                    this.createImpulseSignal(spec, options);
                    break;
            }

            this.activeSignals.set(logicInputId, spec);

            // BusManagerにTest Signalソースを接続
            const busManager = window.busManager;
            if (busManager && spec.nodes.length > 0) {
                const sourceNode = spec.nodes[spec.nodes.length - 1]; // 最後のノードを出力として使用
                busManager.attachSource(logicInputId, sourceNode);
                console.log(`[TestSignalManager] Attached ${type} signal to BusManager for Logic Input: ${logicInputId}`);
            }
    ...
    stop(logicInputId: string): void {
        ...
        const busManager = window.busManager;
        if (busManager) {
            busManager.detachSource(logicInputId);
            console.log(`[TestSignalManager] Detached ${spec.type} signal from BusManager for Logic Input: ${logicInputId}`);
        }
    ...
    private createToneSignal(spec: TestSignalSpec, options: TestSignalOptions): void {
        ...
        // gainNodeへの直接接続は削除 - BusManager.attachSourceで接続される
     ...
     */
    private createNoiseSignal(spec: TestSignalSpec, options: TestSignalOptions): void {
        if (!this.noiseBuffer) {
     ...
     */
    /**
     * Impulse信号作成 (短いパルス 0.6s)
     */
    private createImpulseSignal(spec: TestSignalSpec, options: TestSignalOptions): void {
        ...
        try {
            ...
            if (!logicInput) {
                ...
                logicInput = logicInputManager.add({
                    ...
                    gain: 1.0,
                    enabled: false // Test Signal用のLogic Inputも初期値はfalse
                });
            }
